...
* Viewer: edit `webgl/index.html`, then copy to every `docs/<scenario>/` (east, east-extended, west, quake, storm; data.js stays) and push;
  ...
  site links are explicit `index.html`, so `docs/` also works from `file://`; `make offline` zips it for presentations. Regenerate data with `scripts/07_export_webgl.py --scenario <s> --cell 60`.
